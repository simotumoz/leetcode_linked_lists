...
def copyRandomList(head):
    oldToCopy = {None: None}  # initializing the hashmap

    cur = head  # initializing current node

    while cur:
        copy = Node(cur.val)  # creating a deep copied list and adding cur.val
        oldToCopy[cur] = copy  # adding in the HM the copy node
        cur = cur.next  # incrementing cur

    cur = head  # re-initializing current node
    while cur:

        # using the hashmap to link correctly the nodes
